...
import java.util.function.Consumer; //Это функциональный интерфейс, и поэтому
// его можно использовать в качестве целевого назначения для ссылки на лямбда-выражение или метод.
//Представляет операцию, которая принимает один входной аргумент и не возвращает результата. В отличие от большинства
// других функциональных интерфейсов, Consumer ожидается, что он будет работать через побочные эффекты.
public class UndoStringBuilder {
    private StringBuilder sb;
    private LinkedList<Consumer<StringBuilder>> commands = new LinkedList<>();
//В LinkedList элементы фактически представляют собой звенья одной цепи. У каждого элемента помимо
// тех данных, которые он хранит, имеется ссылка на предыдущий и следующий элемент. По этим ссылкам
// можно переходить от одного элемента к другому.
    //LinkedList сводится к изменению ссылок.
    ...
    public UndoStringBuilder(CharSequence seq) { //Последовательность символов интерфейса
